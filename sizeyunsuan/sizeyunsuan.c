...
int main()
{
    ...
    for(i=0;i<n;i++)
    {
        ...
        else
        {
            ...
            case 2:
                ...
                break;
            case 3:
                formula[i].sum = chufa(sum1[i],formula[i].thirdNumber);
                break;
            }
        }
        //printf("%d %c %d %c %d = %d\n",formula[i].firstNumber,formula[i].fuhao1,formula[i].secondNumber,formula[i].fuhao2,formula[i].thirdNumber,formula[i].sum);
    }
        printf("\n");
    for(j=0;j<n;j++)
    {
    ...
    fenshu = (100/n)*count;
    printf("总共答对了:%d\t总分数为:%f",count,fenshu);
}
void menu()
{
    printf("\n\n");
    printf("\t\t|--------------四则运算---------------|\n");
    printf("\t\t|----------李先灿 201606120073--------|\n");
    printf("\t\t|-------------------------------------|\n");
}
void randomNumber(int n)
{
